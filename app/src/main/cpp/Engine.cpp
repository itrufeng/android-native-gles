...
int nativelib::Engine::InitDisplay() {
    ...
    logger.i("EGL initial result: %d & version: %d.%d", eglInitializeState, majorVersion,
             minorVersion);
 ...
 */
void nativelib::Engine::DestroyDisplay() {
 ...
 */
void nativelib::Engine::DrawFrame() {
...
void nativelib::Engine::Prepare() {
    ...
    this->ReadShaderSource("shader.vert", vertex_shader_source_buffer);
    ...
    this->ReadShaderSource("shader.frag", fragment_shader_source_buffer);
...
void nativelib::Engine::ReadShaderSource(string path, char *out_source) {
...
int32_t
nativelib::Engine::SelectConfigForPixelFormat(EGLDisplay dpy, EGLint const *attrs, int32_t format,
                                              EGLConfig *outConfig) {
    auto logger = Log("NativeGL.main");
